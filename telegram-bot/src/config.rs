...
impl Config {
    pub(crate) fn from_env() -> Config {
        let spending_base_url =
            env::var("SPENDING_API_URL").unwrap_or("http://localhost:8001".into());
        Config {
            ...
            metro_api: Arc::new(MetroScheduleAPI {
                url: env::var("METRO_API_URL")
                    .unwrap_or("http://localhost:8000/next-arrival".into()),
            }),
            enviro_api: Arc::new(EnviroApi {
                url: env::var("ENVIRO_API_URL").unwrap_or("http://localhost:5000".into()),
            }),
