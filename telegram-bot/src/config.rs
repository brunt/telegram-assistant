...
impl Config {
    pub(crate) fn from_env() -> Config {
        Self {
            ...
            metro_api: Arc::new(env::var("METRO_API_URL").map_or(
                MetroScheduleAPI::default(),
                |url| MetroScheduleAPI { url },
            )),
            enviro_api: Arc::new(
                env::var("ENVIRO_API_URL")
                    .map_or(EnviroApi::default(), |url| EnviroApi { url }),
            ),
